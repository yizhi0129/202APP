...
void* main_omp(omp_s_t* args){
	...
	sub (*(args->a), &b);
	c += 500;
	printf("Hello from thread %d %f @a%p @b%p %f %p %f\n", get_rank() ,*(args->a), &(args->a) , &(b),b,&c,c) ;
	return NULL;
}
...
void main() {
 ...
 printf("Outside %f @a%p @b%p %f %p %f\n",a ,&a, &b, b,&c,c) ;

 /*
#pragma omp parallel firstprivate(b) copyin(c)
 {
 b += 290;
 printf("Hello from thread %d %f @a%p @b%p %f %p %f\n", omp_get_thread_num(),a ,&a, &b, b,&c,c) ;
 }
 */
}
